...
class BST:
    ...
    def _find_first_with_prefix(self, current_node, prefix):
        """
        Finds the first element in the BST that starts with the specified prefix.

        Args:
            prefix (str): The prefix to search for.

        Returns:
            str: The first element in the BST that starts with the prefix, or None if no such element exists.
        """
        ...
        if prefix < current_node.value:
            return self._find_first_with_prefix(current_node.left_child, prefix)
        return self._find_first_with_prefix(current_node.right_child, prefix)

    def find_words_with_prefix(self, prefix):
        """
        Finds all words in the BST that start with the specified prefix.

        Args:
            prefix (str): The prefix to search for.

        Returns:
            list: A list of all words in the BST that start with the prefix.
        """
        current_node = self._find_first_with_prefix(self.root, prefix)
        if current_node is None:
            return []
        if current_node.value.startswith(prefix):
            return [current_node.value]
        if prefix < current_node.value:
            return self.find_words_with_prefix(current_node.left_child, prefix)
        return self.find_words_with_prefix(current_node.right_child, prefix)

    def _find_words_with_prefix(self, current_node, prefix, continue_search):
        """
        Helper function for yield_all_with_prefix_until_no_prefix.
        ...
        Yields:
            Node: Each node in the BST with the prefix until it finds a node without the prefix.
        """
        if not continue_search:
            return
        continue_search = True

        if current_node is None or not current_node.value.startswith(prefix):
            continue_search = False
            return
        if current_node.value.startswith(prefix):
            yield current_node

        yield from self.yield_all_with_prefix_until_no_prefix_helper(
            current_node.left_child, prefix, continue_search
        )
        yield from self.yield_all_with_prefix_until_no_prefix_helper(
            current_node.right_child, prefix, continue_search
        )
